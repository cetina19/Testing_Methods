...
describe('Component: Component1 - Mobile (375x667) Viewport ', () => {
  let layout: Layout;
    ...
    it('Text: Save time, ensure accuracy', () => {
      ...
      cy.get('[data-testid="c3e9a7"]').then(($el1) => {
        ...
        cy.get('[data-testid="jj3id3"]').then(($el2) => {
          ...
          const mobileLayout = determinePairLayout(rect1, rect2);
          const mobileAlignment = determinePairAlignment(rect1, rect2, mobileLayout);
          expect(layout).to.equal(mobileLayout);
          expect(alignment).to.equal(mobileAlignment) 
        });
      });      
    });
  });
